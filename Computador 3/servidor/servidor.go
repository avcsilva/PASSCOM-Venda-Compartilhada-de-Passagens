...
func initEtcd() {
    ...
    etcdClient, err = clientv3.New(clientv3.Config{
        Endpoints:   []string{"http://172.16.103.14:2379", "http://172.16.103.13:2379", "http://172.16.103.2:2379"},
...
func acquireLock(session *concurrency.Session, key string) (*concurrency.Mutex, error) {
...
func define_metodo_patch(serv_local *Infos_locais, serv *gin.Engine){
	...
	serv.PATCH("/comprar_rota", func(c *gin.Context) {
		...
		if err != nil {
			fmt.Println("Erro ao criar sessão etcd:", err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
		defer session.Close()
	
		lockKey := fmt.Sprintf("/locks/rota/%s", cliente_req.Rota)
		mutex, err := acquireLock(session, lockKey)
		if err != nil {
			fmt.Println("Erro ao adquirir o bloqueio:", err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
		...
		fonte := c.GetHeader("X-Source")
		if fonte == "servidor" {
			atribuir_rota(serv_local, cliente_req)
			c.JSON(http.StatusOK, gin.H{"status": "rota_comprada"})
			return
		}
	
		_, existe := serv_local.clientes[cliente_req.Id]
		if !existe {
			c.JSON(http.StatusBadRequest, gin.H{"error": "cliente_nao_encontrado"})
			return
		}
	
		_, existe = serv_local.rotas[cliente_req.Rota]
		if !existe {
			c.JSON(http.StatusBadRequest, gin.H{"error": "rota_nao_encontrada"})
			return
		}
	
		atribuir_rota(serv_local, cliente_req)
	
		json_valor, err := json.Marshal(cliente_req)
		if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"status": "rota_comprada"}) // Responde com o status de rota comprada
	})
	...
	serv.PATCH("/cancelar_rota", func(c *gin.Context) {
		...
		fonte := c.GetHeader("X-Source")
		if fonte == "servidor" {
			cancelar_rota(serv_local, cliente_req)
			c.JSON(http.StatusOK, gin.H{"status": "rota_cancelada"})
			return
		}
	
		cliente, existe := serv_local.clientes[cliente_req.Id]
		if !existe {
			c.JSON(http.StatusBadRequest, gin.H{"error": "cliente_nao_encontrado"})
			return
		}
